...
BOOL mount(file_system fs, const char *path)
{
    ...
    if (mountpoints[MAX_MOUNTPOINTS - 1].is_mounted)
        return FALSE;
...
vnode *traverse_path(mounted_fs *mountpoint, const char *path)
{
    ...
    if (!mountpoint->fs.ops->open_root(mountpoint->fs.context, &node))
        return NULL;
    ...
    while (seperator)
    {
        ...
        if (!mountpoint->fs.ops->open(mountpoint->fs.context, &entry, &node))
            return NULL;
    }
...
{
    if (buffer->_size < sector_count * SECTOR_SIZE)
        return NULL;
