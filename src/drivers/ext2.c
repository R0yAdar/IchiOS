...
BOOL ext2_read_bgdt(ext2_context *ctx, uint32_t index, ext2_bgd *out)
{
    ...
    uint32_t bgd_count = ((ctx->_sb.blocks_count + ctx->_sb.blocks_per_group - 1) / ctx->_sb.blocks_per_group);
...
void *ext2_read_block(ext2_context *ctx, ext2_inode *inode, uint64_t block_number, io_buffer *buffer)
{
    ...
    {
        if (io_get_size(buffer) < ctx->block_size)
            return NULL;
        if (inode->direct_bp[block_number] == 0)
            return NULL;
